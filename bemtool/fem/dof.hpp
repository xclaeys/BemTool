...
private:
  ...
  vector<typename Trait::Nloc>     elt_to_dof;
  vector< vector<N2> >             dof_to_elt;
  int                              nb_elt;
  int                              nb_dof;
  int                              offset; 
...
public:    
  ...
  const vector<N2>& ToElt(const int& j) const {
...
template<>
typename Dof<P2_1D>::Trait::R3loc
Dof<P2_1D>::operator()(const int& j) const {
  Elt<Trait::dim> e = (*mesh_p)[j];
  typename Dof<P2_1D>::Trait::R3loc x;
  x[0] = e[0];
  x[1] = e[1];
  x[2] = e[2];
  return x;
...
template<>
typename Dof<P2_2D>::Trait::R3loc
Dof<P2_2D>::operator()(const int& j) const {
  Elt<Trait::dim> e = (*mesh_p)[j];
  typename Dof<P2_2D>::Trait::R3loc x;
  x[0] = e[0];
  x[1] = e[1];
  x[2] = e[2];
  x[3] = 0.5*(x[1]+x[2]);
...
public:    
  ...
  typename Trait::R3loc operator()(const int& j) const {
    Elt2D e = (*mesh_p)[j];
    typename Dof<RT0_2D>::Trait::R3loc x;
    x[0] = 0.5*(x[1]+x[2]);
    x[1] = 0.5*(x[2]+x[0]);
    x[2] = 0.5*(x[0]+x[1]);
    return x;
